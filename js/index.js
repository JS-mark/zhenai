...
$(window).load(function() {
		...
		afterLoad: function(anchorLink, index) {
			...
			if (index == 6) {
				...
				$('.sd_down_app_down .left')
					.find('i')
					.eq(1)
					.animate({ left: '25%' }, 1500)
				$('.sd_down_app_down .right')
					.find('i')
					.eq(0)
					.animate({ left: '22%' }, 1500)
					.rotate({
						bind: {
							click: function() {
								$(this).rotate({ angle: 0, animateTo: 360 })
	...
	sd_$.Bvideo(
		'#sd_video',
		'http://hc.yinyuetai.com/uploads/videos/common/C75F0164CA55138163AB00C8004538CB'
	)
	//调用遮罩层
	...
	$('.sd_img_box_w').mouseleave(function() {
		$('.sd_img_box_w ' + 'li i').css({ opacity: '0' })
	})
	//首页底部按钮进入事件
	$('.sd_index_footer_btn')
		.find('span')
		.mouseenter(function() {
			$(this)
				.addClass('sd_index_footer_btn_selected')
				.siblings('span')
				.removeAttr('class')
		})
	//tab栏img盒子
